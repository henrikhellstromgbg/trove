...
| Capture, Library and Ask | Exists | Browser capture uses `/api/ingest`. Ask is project-scoped and persists conversations, messages and citations; a Chat archive page (`/p/[slug]/chats`, in the left nav) lists, resumes and deletes threads (`DELETE /api/ask`). |
