...
| Capture, Library and Ask | Exists | Browser capture uses `/api/ingest`. Ask is project-scoped and persists conversations, messages and citations, and now surfaces them: the right panel is tabbed sources/chats to list, resume, start and delete threads (`DELETE /api/ask`). |
| Sources | Partial | RSS, web scrape and Slack poll exist. The Tauri app runs `mail_folder` (local mbox) and `folder_watch` locally: it reads only registry-approved paths, posts through `/api/ingest` with the ingest token and an explicit per-source projectId, and stays idempotent via local checkpoints + externalId. The daemon now fetches its approved registry from `GET /api/sources/local` (token-auth), falling back to the local file offline; ingest tokens are issued/revoked through `/api/ingest-tokens`. A background timer syncs the local sources on an interval (`TROVE_LOCAL_SYNC_INTERVAL_SECS`, default 300s), so runs no longer need a tray click. Local sources are now created from the web form (`/p/[slug]/sources/new`) and tokens issued from `/p/[slug]/settings`, so the runtime is self-serve. Cloud/Gmail fetchers remain. |
| Pipelines | Exists | List, plain-language creation, detail, pause, run-now, history, and seeded Friday weekly digest exist. Starter templates (Morning brief, Friday weekly summary) install from a picker on `/pipelines/new` with an optional email toggle. |
...
- **Cloud sources** (`rss`, `web_scrape`, `slack_channel`, and later `youtube_channel`) are polled by the existing Inngest `sync-due-sources` function. It loads enabled cloud sources whose `next_run` is due, calls a per-kind fetcher, and emits `item/captured` for new external items. On a *repeated* failure (the previous run also failed) `next_run` is pushed to at least `completedAt + TROVE_SOURCE_ERROR_BACKOFF_SECS` (default 3600s, floored at 60s) instead of retrying at full cron cadence, so a persistently failing source stops hammering the upstream API; a one-off blip keeps the normal schedule.
...
3. **Richer triggers.** Done (additive). Alongside cron, a pipeline can opt into `runOnNewItem`: when an item becomes ready (`item/ready`, emitted after processing), `runEventPipelines` runs any enabled pipeline in that project whose filter the item matches and that is off a 10-minute cooldown (debouncing bursts and coincident cron runs, keyed on `lastRunAt`). The matching + selection logic is pure and tested in `lib/pipelines/triggers.ts`; the cron path is unchanged.
...
The weekly digest is already implemented as one seeded pipeline per project. It remains a normal pipeline rather than a separate subsystem. The seed runs Friday at 15:00 to match the starter template. The Digest surfaces (page + dashboard panel) find it by `templateKey = "weekly-summary"` (the shared `WEEKLY_DIGEST_TEMPLATE_ID`), not by the display name, so a rename or a user-made pipeline can't hijack it.
...
8. **Product UI.** Phases 1–7 are done on the backend, but several capabilities are reachable only by API. Build the front doors, per `docs/ui-layout-v2.md` (behavior + screen inventory) and `docs/trove-project-flow-architecture.html` (visual). Sequenced by what a ready backend already unlocks:
   ...
   3. **Item detail actions** — Done. The item viewer has an actions card: rename and retag (`PATCH /api/items/[id]`, added this round), reprocess (`POST /[id]/reprocess`, added this round — clears chunks, resets to pending, replays the worker), move/copy (`/[id]/move`) and trash (`/api/items/trash`, the control that fills the trash view).
...
- **Move or copy across projects.** Done. `POST /api/items/[id]/move` with `{ toProjectId }` (destination required and validated as owned; the user never changes, so only the project wall is crossed, deliberately). A manual item (`source_id = null`) is moved: its row and chunks are re-scoped to the destination and it is dropped from any topic cluster in the origin project (`array_remove`). A source-attributed item is copied instead (a source belongs to one project, so moving would create a cross-project `source_id`): the copy carries the item's type/source and extracted text, detached from the source (`source_id`/`external_id` null), does not share the blob, and is reprocessed in the destination via `item/captured`; the original is untouched. Only `ready` items are eligible (`lib/items/move.ts`, tested in `tests/item-move.test.ts`).
...
- Slack-pollen följer nu markörpaginering (`has_more` / `next_cursor`) i både `conversations.history` och `conversations.replies`, så ett aktivt fönster med fler än 200 meddelanden inte tappar de äldsta (tak på 20 sidor, loggas om det nås). Första sidan bär `oldest`; efterföljande sidor bär bara markören.
